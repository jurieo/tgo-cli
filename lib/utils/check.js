 ...
 * @LastEditTime: 2019-09-09 16:08:31
 * @Description: 检查版本
 */
...
function checkPackageVersion(url) {
  return new Promise((resolve, reject) => {
    ...
    latestVersion("tgo-cli").then(version => {
      if (semver.lte(version, requiredVersion)) {
        ...
        console.log(
          chalk.red(
            `发现新版 TGO-cli v${requiredVersion}，你正在使用的 TGO-cli 版本为 v${requiredVersion}.\n请更新至最新版后再重试. \n\n>> npm update tgo-cli -g`
          )
        );
        process.exit(1);
      }
  ...
  //         process.exit(1);
  //       }
  //       console.log(chalk.red("版本为最新版！"));
  //     } else {
  //       spinner.clear();
  //       console.log(chalk.red("未能获取版本信息！"));
  //       reject(error);
  //       process.exit(1);
  //     }
  //   });

...
function checkPackageVersion(url) {
  return new Promise((resolve, reject) => {
    spinner.start("Checking TGO-cli version");
    request(url, function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        if (semver.lte(version, requiredVersion)) {
          ...
          console.log(
            chalk.red(
              `You are using TGO-cli v${requiredVersion}, But the latest version is v${version}.\nPlease upgrade your TGO-cli version. \n\n>> npm update tgo-cli -g`
            )
          );
          process.exit(1);
        }
        ...
        spinner.clear();
        console.log(
          chalk.red("Failed to obtain version information through NPM!")
        );
        reject(error);
        process.exit(1);
      }
